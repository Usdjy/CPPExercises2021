...
void testBGRToGray() {
    ...
    //  реализуйте эту функцию, преобразующую картинку в черно-белую
    cv::Mat gray = convertBGRToGray(img);

    //  и удостоверьтесь что результат выглядит так как вы ожидаете, если нет - спросите меня
...
void testSobel(const std::string &name) {
    ...
    //  реализуйте функцию считающую применение оператора Собеля к картинке
    ...
    cv::Mat dx = convertDXYToDX(dxy); //  реализуйте функцию которая вытаскивает силу производной по x (ее абсолютное значение)
    //  и удостоверьтесь что результат выглядит так как вы ожидаете, если нет - спросите меня
    cv::imwrite("lesson05/resultsData/" + name + "_dx.jpg", dx);

    cv::Mat dy = convertDXYToDY(dxy); //  реализуйте функцию которая вытаскивает силу производной по y (ее абсолютное значение)
    //  и удостоверьтесь что результат выглядит так как вы ожидаете, если нет - спросите меня
    cv::imwrite("lesson05/resultsData/" + name + "_dy.jpg", dy);

    cv::Mat gradientStrength = convertDXYToGradientLength(dxy); //  реализуйте функцию которая считает силу градиента в каждом пикселе
