...
cv::Mat drawContours(int rows, int cols, std::vector<std::vector<cv::Point>> contoursPoints) {
    ...
    int n=blackImage.rows;int m=blackImage.cols;
...
void test(std::string name, std::string k) {
    ...
    //  01 выполните бинарный трешолдинг картинки, прочитайте документацию по функции cv::threshold и выберите значения аргументов
    cv::Mat binary;
    ...
    cv::Mat binaryhold;
    cv::erode(binary, binaryhold, cv::getStructuringElement(cv::MORPH_RECT, cv::Size(4, 4)));
   ...
   binary = binary_eroded;
    ...
    int n=binary1.rows;int m=binary1.cols;
    /*for(int i=0;i<n;++i) for(int j=0;j<m;++j)
        {
        if(i<=5 || (n-i)<=5 || j<=5 || (m-j)<=5) {cout<<i<<" i "<<j<<" j "<<endl;binary1.at<float>(i,j)=255;}
        }*/
    cv::Mat binary2;
    cv::adaptiveThreshold(img, binary2, 255, cv::BORDER_REPLICATE, cv::THRESH_BINARY, 7, 10);
    cv::imwrite(out_path + "/04_erode+adaptive_threshold_original.jpg", binary2);
...
//        // TODO прочитайте документацию cv::rectangle чтобы нарисовать прямоугольник box с толщиной 2 и черным цветом (обратите внимание какие есть поля у box)
        cv::rectangle(imgWithBoxes, box, blackColor, 2, LINE_8);
    }
