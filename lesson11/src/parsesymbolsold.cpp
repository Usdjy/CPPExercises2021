...
cv::Mat generateImage4(std::string text, int width=128, int height=128) {
    ...
    int font = randFont4();
    double fontScale = randFontScale4();
    int thickness = randThickness4();
    cv::Scalar color = randColor24();
...
HoG normalize4(HoG a)
{
    double sum=0.0;for(auto h:a) sum+=h;
    for(auto &h:a) h/=sum;
    return a;
}
double dist4(hogt a,hogt b)
{
    double ans=0;
    for(int i=0;i<a.size();++i) ans+=abs(a[i]-b[i]);
    return ans;
}
HoG buildHoG4(cv::Mat grad_x, cv::Mat grad_y) {
...
HoG buildHoG4(cv::Mat originalImg) {
    ...
    HoG hog = buildHoG4(grad_x, grad_y);
    return hog;
}

HoG buildHoG24(cv::Mat originalImg) {
    cv::Mat img = originalImg.clone();
...
vector <vector <hogt> > v;
void generateAllLetters4() {
    ...
    for (char letter = 'a'; letter <= 'z'; ++letter) {
        ...
        for (int sample = 1; sample <= NSAMPLES_PER_LETTER; ++sample) {
            ...
            cv::Mat img = generateImage4(text);
...
void go14() {
    ...
    for (char letter = 'a'; letter <= 'z'; ++letter) {
        ...
        for (int sampleA = 1; sampleA <= NSAMPLES_PER_LETTER; ++sampleA) {
            ...
            //cout<<" ytgrf "<<endl;
            HoG hogA = buildHoG4(a);
            //cout<<letter<<endl;
            hogt ans=normalize4(hogA);
            //cou(ans);
...
string ch4(cv::Mat img)
{
    string ans;
    hogt u=normalize4(buildHoG24(img));
    //cou(u);
    pair <int,int> el={-1,-1};
    for(int i=0;i<26;++i) for(int j=0;j<NSAMPLES_PER_LETTER;++j)
        {
            if(el.first==(-1) || dist4(u,v[el.first][el.second])>dist4(u,v[i][j])) el={i,j};
        }
    double d=dist4(u,v[el.first][el.second]);
    //cout<<d<<endl;
    if(d>=0.2) return ans;
