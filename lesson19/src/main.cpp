...
#pragma omp critical
        ...
        for(int i=((n+nthr-1)/nthr)*id;i<min(n,((n+nthr-1)/nthr)*(id+1));++i) {int value = data[i];
            if (value > max11) { // если текущее значение больше самого большого
                max21 = max11;    // то число которое раньше было самым большим - становится вторым по величине
                max11 = value;   // а текущее значение становится самым большим
            } else {            // иначе:
                if (value > max21) { // если текущее значение больше хотя бы второго по величине
                    max21 = value;   // то заменяем его
                }
            }}
#pragma omp critical
        {
...
#pragma omp parallel for
    for(int i=0;i<n;++i)
    {
        int tid = omp_get_thread_num();ans[i]=tid;
    }
